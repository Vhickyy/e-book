...
export const notAuthorGuard: CanActivateFn = (route, state) => {
    ...
    switchMap(([user, loading]) => {
      if (!user && loading) {
        ...
        return combineLatest([store.select(selectUser), store.select(selectLoading)]).pipe(
          ...
          map(([user]) => {
            if (user?.role == 'author') {
              return router.createUrlTree([`/author/${user?.name}/${user?._id}`]);
            }
            return true;
          })
        );
        ...
        if (!loading && user?.role == 'author') {
          return of(router.createUrlTree([`/author/${user?.fullname}/${user?._id}`]));
        }
        return of(true);
      }
    })
